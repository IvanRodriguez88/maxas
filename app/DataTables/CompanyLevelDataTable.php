...
class CompanyLevelDataTable extends DataTable
{
    /**
     ...
     *
     * @param \App\Models\CompanyLevel $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(CompanyLevel $model): QueryBuilder
    {
        return $model->select(
			'company_levels.id',
            'company_levels.name',
            'company_levels.description',
			'company_levels.created_at',
            'company_levels.updated_at',
			'company_levels.is_active',
		)
		->newQuery();
    }
    ...
    public function html(): HtmlBuilder
    {
        return $this->builder()
                ...
                ->setTableId('company_levels-table')
    ...
    public function getActions($row){
        $result = null;
        if (auth()->user()->hasPermissions("company_levels.edit")) {
            $result .= '
                <a title="Editar" href='.route("company_levels.edit", $row->id).' class="btn btn-outline-secondary btn-icon ps-2 px-1">
        ...
        if (auth()->user()->hasPermissions("company_levels.destroy")) {
            $result .= '
    ...
    public function getColumns(): array
    {
        ...
        if (auth()->user()->hasPermissions("company_levels.edit") ||
            auth()->user()->hasPermissions("company_levels.create") ||
            auth()->user()->hasPermissions("company_levels.destroy")) {
    ...
    {
        return 'CompanyLevels_' . date('YmdHis');
    }
}
