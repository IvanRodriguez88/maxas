...
class ReturnBaseDataTable extends DataTable
{
    /**
     ...
     *
     * @param \App\Models\ReturnBase $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(ReturnBase $model): QueryBuilder
    {
        return $model->select(
			'return_bases.id',
            'return_bases.name',
            'return_bases.description',
			'return_bases.created_at',
            'return_bases.updated_at',
			'return_bases.is_active',
		)
		->newQuery();
    }
    ...
    public function getActions($row){
        $result = null;
        if (auth()->user()->hasPermissions("return_bases.edit")) {
            $result .= '
                <a title="Editar" href='.route("return_bases.edit", $row->id).' class="btn btn-outline-secondary btn-icon ps-2 px-1">
        ...
        if (auth()->user()->hasPermissions("return_bases.destroy")) {
            $result .= '
    ...
    public function getColumns(): array
    {
        ...
        if (auth()->user()->hasPermissions("return_bases.edit") ||
            auth()->user()->hasPermissions("return_bases.create") ||
            auth()->user()->hasPermissions("return_bases.destroy")) {
    ...
    {
        return 'ReturnBases' . date('YmdHis');
    }
}
