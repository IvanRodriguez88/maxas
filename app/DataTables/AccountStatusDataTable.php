...
class AccountStatusDataTable extends DataTable
{
    /**
     ...
     *
     * @param \App\Models\AccountStatus $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(AccountStatus $model): QueryBuilder
    {
        return $model->select(
			'account_statuses.id',
            'account_statuses.name',
            'account_statuses.description',
			'account_statuses.created_at',
            'account_statuses.updated_at',
			'account_statuses.is_active',
		)
		->newQuery();
    }
    ...
    public function html(): HtmlBuilder
    {
        return $this->builder()
                ...
                ->setTableId('account_statuses-table')
    ...
    public function getActions($row){
        $result = null;
        if (auth()->user()->hasPermissions("account_statuses.edit")) {
            $result .= '
                <a title="Editar" href='.route("account_statuses.edit", $row->id).' class="btn btn-outline-secondary btn-icon ps-2 px-1">
        ...
        if (auth()->user()->hasPermissions("account_statuses.destroy")) {
            $result .= '
    ...
    public function getColumns(): array
    {
        ...
        if (auth()->user()->hasPermissions("account_statuses.edit") ||
            auth()->user()->hasPermissions("account_statuses.create") ||
            auth()->user()->hasPermissions("account_statuses.destroy")) {
    ...
    {
        return 'AccountStatuses_' . date('YmdHis');
    }
}
