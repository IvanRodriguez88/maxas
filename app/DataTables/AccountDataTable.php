...
class AccountDataTable extends DataTable
{
    ...
    public function getActions($row){
        $result = null;
        if (auth()->user()->hasPermissions("accounts.edit")) {
            $result .= '
                <a title="Editar" href='.route("accounts.edit", $row->id).' class="btn btn-outline-secondary btn-icon ps-2 px-1">
        ...
        if (auth()->user()->hasPermissions("accounts.destroy")) {
            $result .= '
    ...
    public function getColumns(): array
    {
        ...
        if (auth()->user()->hasPermissions("accounts.edit") ||
            auth()->user()->hasPermissions("accounts.create") ||
            auth()->user()->hasPermissions("accounts.destroy")) {
