...
class CurrencyTypeDataTable extends DataTable
{
     ...
     *
     * @param \App\Models\CurrencyType $model
    ...
    public function query(CurrencyType $model): QueryBuilder
    {
        return $model->select(
			'currency_types.id',
            'currency_types.name',
            'currency_types.symbol',
            'currency_types.description',
			'currency_types.created_at',
            'currency_types.updated_at',
			'currency_types.is_active',
		)
		->newQuery();
    }
    ...
    public function getActions($row){
        $result = null;
        if (auth()->user()->hasPermissions("currency_types.edit")) {
            $result .= '
                <a title="Editar" href='.route("currency_types.edit", $row->id).' class="btn btn-outline-secondary btn-icon ps-2 px-1">
        ...
        if (auth()->user()->hasPermissions("currency_types.destroy")) {
            $result .= '
    ...
    public function html(): HtmlBuilder
    {
        ...
        ->setTableId('currency_types-table')
    ...
    public function getColumns(): array
    {
        ...
        if (auth()->user()->hasPermissions("currency_types.edit") ||
            auth()->user()->hasPermissions("currency_types.create") ||
            auth()->user()->hasPermissions("currency_types.destroy")) {
    ...
    {
        return 'CurrencyTypes_' . date('YmdHis');
    }
}
