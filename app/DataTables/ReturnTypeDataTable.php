...
class ReturnTypeDataTable extends DataTable
{
    /**
     ...
     *
     * @param \App\Models\ReturnType $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(ReturnType $model): QueryBuilder
    {
        return $model->select(
			'return_types.id',
            'return_types.name',
            'return_types.description',
			'return_types.created_at',
            'return_types.updated_at',
			'return_types.is_active',
		)
		->newQuery();
    }
    ...
    public function getActions($row){
        $result = null;
        if (auth()->user()->hasPermissions("return_types.edit")) {
            $result .= '
                <a title="Editar" href='.route("return_types.edit", $row->id).' class="btn btn-outline-secondary btn-icon ps-2 px-1">
        ...
        if (auth()->user()->hasPermissions("return_types.destroy")) {
            $result .= '
    ...
    public function getColumns(): array
    {
        ...
        if (auth()->user()->hasPermissions("return_types.edit") ||
            auth()->user()->hasPermissions("return_types.create") ||
            auth()->user()->hasPermissions("return_types.destroy")) {
    ...
    {
        return 'ReturnTypes' . date('YmdHis');
    }
}
