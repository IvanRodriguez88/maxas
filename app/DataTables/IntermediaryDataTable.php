...
class IntermediaryDataTable extends DataTable
{
    /**
     ...
     *
     * @param \App\Models\Intermediary $model
    ...
    public function html(): HtmlBuilder
    {
        return $this->builder()
                ...
                ->setTableId('intermediaries-table')
    ...
    public function getActions($row){
        $result = null;
        if (auth()->user()->hasPermissions("intermediaries.edit")) {
            $result .= '
                <a title="Editar" href='.route("intermediaries.edit", $row->id).' class="btn btn-outline-secondary btn-icon ps-2 px-1">
        ...
        if (auth()->user()->hasPermissions("intermediaries.destroy")) {
            $result .= '
    ...
    public function getColumns(): array
    {
        ...
        if (auth()->user()->hasPermissions("intermediaries.edit") ||
            auth()->user()->hasPermissions("intermediaries.create") ||
            auth()->user()->hasPermissions("intermediaries.destroy")) {
    ...
    {
        return 'Intermediarys_' . date('YmdHis');
    }
}
