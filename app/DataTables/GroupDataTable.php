...
class GroupDataTable extends DataTable
{
    ...
    public function getActions($row){
        $result = null;
        if (auth()->user()->hasPermissions("groups.edit")) {
            $result .= '
                <a title="Editar" href='.route("groups.edit", $row->id).' class="btn btn-outline-secondary btn-icon ps-2 px-1">
        ...
        if (auth()->user()->hasPermissions("groups.destroy")) {
            $result .= '
    ...
    public function getColumns(): array
    {
        ...
        if (auth()->user()->hasPermissions("groups.edit") ||
            auth()->user()->hasPermissions("groups.create") ||
            auth()->user()->hasPermissions("groups.destroy")) {
