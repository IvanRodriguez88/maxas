...
class BankSeparationDataTable extends DataTable
{
    /**
     ...
     *
     * @param \App\Models\BankSeparation $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(BankSeparation $model): QueryBuilder
    {
        return $model->select(
			'bank_separations.id',
            'bank_separations.name',
            'bank_separations.description',
			'bank_separations.created_at',
            'bank_separations.updated_at',
			'bank_separations.is_active',
		)
		->newQuery();
    }
    ...
    public function html(): HtmlBuilder
    {
        return $this->builder()
                ...
                ->setTableId('bank_separations-table')
    ...
    public function getActions($row){
        $result = null;
        if (auth()->user()->hasPermissions("bank_separations.edit")) {
            $result .= '
                <a title="Editar" href='.route("bank_separations.edit", $row->id).' class="btn btn-outline-secondary btn-icon ps-2 px-1">
        ...
        if (auth()->user()->hasPermissions("bank_separations.destroy")) {
            $result .= '
    ...
    public function getColumns(): array
    {
        ...
        if (auth()->user()->hasPermissions("bank_separations.edit") ||
            auth()->user()->hasPermissions("bank_separations.create") ||
            auth()->user()->hasPermissions("bank_separations.destroy")) {
    ...
    {
        return 'BankSeparations_' . date('YmdHis');
    }
}
