...
class BankDataTable extends DataTable
{
    ...
    public function getActions($row){
        $result = null;
        if (auth()->user()->hasPermissions("banks.edit")) {
            $result .= '
                <a title="Editar" href='.route("banks.edit", $row->id).' class="btn btn-outline-secondary btn-icon ps-2 px-1">
        ...
        if (auth()->user()->hasPermissions("banks.destroy")) {
            $result .= '
    ...
    public function getColumns(): array
    {
        ...
        if (auth()->user()->hasPermissions("banks.edit") ||
            auth()->user()->hasPermissions("banks.create") ||
            auth()->user()->hasPermissions("banks.destroy")) {
