                        ...
                        @foreach($modules as $key => $module)
                            @if($module->parent_id == null || $module->hasSubmodules())
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link {{$key == 0 ? 'active' : ''}}" id="pills-{{$module->name}}-tab" data-bs-toggle="pill" data-bs-target="#pills-{{$module->name}}" type="button" role="tab" aria-controls="pills-{{$module->name}}" aria-selected="true">{{$module->name}}</button>
                                </li>
                            @endif
                        @endforeach
                    </ul>
                                            ...
                                            <div class="d-flex align-items-start">
                                                <div class="nav flex-column nav-pills me-3" id="v-pills-tab2" role="tablist" aria-orientation="vertical">
                                                    @foreach($module->submodules as $key2 => $submodule)
                                                        <button class="nav-link {{$key2 == 0 ? 'active' : ''}}" id="v-pills-{{$submodule->name}}-tab" data-bs-toggle="pill" data-bs-target="#v-pills-{{$submodule->name}}" type="button" role="tab" aria-controls="v-pills-{{$submodule->name}}" aria-selected="true">{{$submodule->name}}</button>
                                                    @endforeach
                                                </div>
